...
impl AppConfig {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        if rpc_url.is_empty() {
            return Err(ConfigError::InvalidRpcUrl("RPC URL cannot be empty".to_string()));
        }

        let rpc_user = std::env::var("BITCOIN_RPC_USER").ok().filter(|s| !s.is_empty());
        let rpc_password = std::env::var("BITCOIN_RPC_PASSWORD").ok().filter(|s| !s.is_empty());
        let rpc_cookie = std::env::var("BITCOIN_RPC_COOKIE")
            .ok()
            .filter(|s| !s.is_empty())
            .map(PathBuf::from);

        let db_path_str = std::env::var("WALLET_DB_PATH")
            .unwrap_or_else(|_| "./data/wallet.sqlite".to_string());
